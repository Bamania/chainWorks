...
router.post('/get-ongoing-proposals', authenticateToken, async (req, res) => {
  try {
    ...
    const proposalidinstring = proposals[0]._id.toString()
    // const proposalid=Number(`0x${proposalidinstring}`);
    const partialHex = proposalidinstring.substring(0, 8);
    const proposalid = parseInt(partialHex, 16);
