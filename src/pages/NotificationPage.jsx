...
const NotificationsPage = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const response = await fetch('https://chainworks.onrender.com/api/notifications');
  ...
  const handleApprove = async (notificationId) => {
    try {
      const response = await fetch(`https://chainworks.onrender.com/api/approve/${notificationId}`, {
        method: 'POST',
        headers: {
