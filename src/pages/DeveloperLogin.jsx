...
const navigate=useNavigate();
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://chainworks.onrender.com/api/login', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-sm w-full">
        ...
        <div className="text-center mt-6">
          <p>Don't have an account? <a href="DeveloperSignup" className="text-sm text-blue-500 hover:text-blue-700">Register Now</a></p>
        </div>
      </div>
    </div>
  );
