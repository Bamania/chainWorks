...
const FreelancherJobList = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        const response = await fetch('https://chainworks.onrender.com/api/allprojects', {
          method: 'GET',
          headers: {
  ...
  const handleApply = async (job) => {
    ...
    try {
      const response = await fetch(`https://chainworks.onrender.com/api/apply/${job._id}`, { // developer applying for job 1 ! can be distinguished using job Id
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
